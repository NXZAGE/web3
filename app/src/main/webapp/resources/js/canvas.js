...
function drawArea(r) {
    ...
    const rect = new Konva.Rect({
        ...
        height: (R / 2) * SCALE,  // Высота от 0 до -R/2
...
function setupRadiusWatcher() {
  $("#rInput").change(function() {
    console.log("rInput on changed");
    const val = $(this).val();
    if (isRadiusValid(parseFloat(val))) {
      console.log("new R is valid. Redrawing...");
      drawVisualization(parseFloat(val));
    } else {
...
function drawExistedPoints(r) {
  HITLOG.forEach((log) => {
    if (parseFloat(log.r) == r) {
      drawPoint(parseFloat(log.x), parseFloat(log.y), r);
    }
  });
}
